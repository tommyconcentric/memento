...
- **Recoloring the logo:** the icon color *picker* (and the About sheet that hosted it) was removed — it never worked on the Mac. `LogoMark`/`LogoWordmark` still take a `colorScheme: LogoColorScheme` and views showing the logo still read `@AppStorage("logoColorScheme")`, purely to keep honoring a scheme a user picked in an earlier version (their Home Screen icon may still carry it). The alternate `AppIcon-*.appiconset`s and `Scripts/generate_icons.swift` remain for that same legacy reason; don't add new schemes.
