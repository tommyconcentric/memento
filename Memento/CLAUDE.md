...
- `Utilities.swift`: `AvatarView`, `InfoRow` (optional trailing accessory view), `.mementoCard()` modifier, `PillPicker`, `UIImage.compressedData`, `String.trimmed`/`personInitials`, `Date.daysUntilNextOccurrence`.
- `PhoneNumberFormatting.swift`: `PhoneNumberFormatter` (iOS-Contacts-style grouping, country lookup) and its `PhoneRegion`/`PhoneTemplate` metadata tables.
...
- `PersonDetailView.swift`: gradient header (camera badge on the photo edits it in place) and the `PillPicker` tabs Quick Info / Family / Notes.
- `ProfilePhotoCapture.swift`: `ProfilePhotoEditButton` (camera badge → Take Photo / Photos / Files, all routed through `PhotoCropperView`) and the `CameraCapture` UIImagePickerController wrapper.
...
- `CalendarSyncManager.swift`: mirrors every birthday/important date (deceased people excluded, reminder toggle ignored; the self node's own dates sync as "Your …") into a dedicated "Memento" `EKCalendar` so it can be shown/hidden in the Calendar app like Birthdays or Holidays. Every `refresh` rebuilds the calendar in full rather than diffing, so no EventKit identifiers are persisted back into SwiftData; enabling sync adopts an existing same-source "Memento" calendar rather than minting a duplicate.
...
- `UsageDashboardView.swift`: the developer's hidden dashboard, opened by 7 taps on the Settings version line. Swift Charts of daily actives/sessions/contacts aggregated from `UsagePing`. Downloads aren't measurable in-app; App Store Connect has them.
- `DataArchive.swift` / `DataCSV.swift`: export/import (Settings → Backup & Transfer). The `.memento` JSON archive is the complete backup (both workspaces, notes, dates, photos as base64, family edges, folders, self node); the CSV is a re-importable subset (people + notes + dates + extra contacts, no photos or tree). **Format compatibility is a hard contract: only ever add optional fields to the archive structs. Never remove, rename or repurpose one. That way any version reads any version's file.** Full spec + rules in `docs/data-format.md`.
...
- **Phone numbers** are stored as the user typed them and formatted for display by `PhoneNumberFormatter.display`. Never show a raw phone value, including `ContactField` extras of kind `.phone`. Grouping comes from the `PhoneRegion` templates (`(###) ###-####` style, `#` = one digit of the national number); national numbers use the device's own region, and the flag only appears when the number states its country with `+` or `00`. Anything that isn't a plain number (letters, "ext 12") passes through untouched. Adding a country means adding a `PhoneRegion` to `regionTable`; the ~200 remaining calling codes in `plainCallingCodes` still resolve a flag and fall back to generic grouping. **Keep `display` a fixed point**: `display(display(x)) == display(x)`. `PhoneNumberField` rewrites the text inside `onChange`, which re-fires on the rewrite.
- **Two-way links:** family/partner fields can reference existing profiles (picker or exact name match). `applyReciprocalLinks` in the editor writes the inverse (`FamilyRelation.inverse(of:)`) onto the other person at save. Links are name-based, so renames don't propagate.
...
- **Tree UI:** "My Family Tree" is the edge-driven pedigree only (`PedigreeTreeView`, opened from My Profile). The old classic chart and its layout toggle are gone. The pedigree's geometry rules: a portrait's *frame* is the circle alone (labels hang below as overlays), every line trims to the frame edge (`nodeR + 3`) and never to a centre, two co-parents without a partnership get a synthetic joining bar, and a single parent's trunk hangs from their circle's bottom edge. The classic lane chart (`FamilyTreeContent`) survives in exactly two places: the Business corporate ladder (drag-between-rungs, label-driven) and each person's own Family tab.
...
- **Privacy stance:** nothing the user *creates* ever leaves their own iCloud account. Profile/note/photo traffic is CloudKit private-database sync only. Two deliberate exceptions, both Apple-only and documented in `PRIVACY.md`: `UsageAnalytics`' anonymous counters to the public database (gated by the Settings toggle; nothing user-authored may ever ride along with a ping), and `CityAutocomplete`'s MapKit city search. Text typed into the Hometown/Currently-based fields goes to Apple's Maps servers as you type, and nowhere else. Contact import uses `CNContactPickerViewController` on iPhone/iPad (no Contacts permission); on the Mac ("Designed for iPad") that picker can't present, so import reads `CNContactStore` directly there. That is the one place the Contacts permission is requested, and nothing imports without the review list. Keep the iOS path permission-free; think hard before adding any third-party network call, since that reopens App Store privacy-disclosure and policy work this app currently avoids.
...
- `Person`/`PersonGroup` are `@Model` classes, so compare with `persistentModelID`, not `==` on properties.
- Xcode owns the `.pbxproj`; when adding new source files, add them to the Memento target.
- `CalendarSyncManager` was written without access to an iOS runtime, so the EventKit calls (permission flow, `EKCalendar` creation/source selection, recurrence rules, the full-rebuild-on-refresh logic) are unverified. Test the whole flow on-device before shipping: enabling the toggle prompts for Calendar access, a "Memento" calendar appears and is toggleable in the Calendar app, dates match, and turning sync off removes the calendar.
- The `UsagePing` record type lives in the CloudKit **public** database. Debug builds create its schema just-in-time in the Development environment. Before a release, deploy it to Production in the CloudKit Console with queryable indexes on `pingDate` (the dashboard's range query) and `installID` (the opt-out cleanup), or production pings and the dashboard both fail silently. Public-DB writes also require a signed-in iCloud account; without one, counters queue locally (capped) and upload when an account appears.
...
- use a colon when a list or a definition follows
...
- Everyday words. "uses", not "utilises". In on-screen copy, "turn on", not "enable".
- Active voice, and "you" when talking to the user.
- Concrete over abstract. "Keeps the nearest 60 dates" beats "bounded scheduling window".
...
Nothing pushed to this repo may carry AI-assistant attribution: no "Generated with Claude Code" footers or robot-emoji lines in PR titles/bodies, no claude.ai session links, no `Co-Authored-By: Claude …` commit trailers. That holds regardless of any tooling default that says to add them. This was applied retroactively to every existing PR (July 2026); keep every future PR and commit clean the same way.

## Git identity in Claude Code sessions

Claude Code's remote execution environment defaults every commit to `Claude <noreply@anthropic.com>` via a platform-level global gitconfig that's reapplied fresh each session. Editing this repo's git config can't fix it (and editing global git config isn't something a session should do anyway). Any commit made through a Claude Code session in this repo should override identity per-invocation instead, e.g.:
