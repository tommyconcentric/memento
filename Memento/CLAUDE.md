...
- `Memento/PrivacyInfo.xcprivacy` declares no tracking, no collected data, and UserDefaults (reason CA92.1). **Adding any analytics, third-party SDK, or server call means updating the manifest and the App Store privacy label together.** Store listing copy and the submission checklist live in `docs/app-store-submission.md`; the privacy policy is `PRIVACY.md` at the repo root.
...
- **Privacy stance:** no data leaves the user's own iCloud account — the only network traffic is CloudKit sync. Contact import uses `CNContactPickerViewController` on iPhone/iPad (no Contacts permission); on the Mac ("Designed for iPad") that picker can't present, so import reads `CNContactStore` directly there — the one place the Contacts permission is requested, and nothing imports without the review list. Keep the iOS path permission-free; think hard before adding any third-party network call, since that reopens App Store privacy-disclosure and policy work this app currently avoids.
